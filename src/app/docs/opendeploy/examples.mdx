...
```bash
# From workspace root
opendeploy run --all --env preview --dry-run --json
```
...
Add variables under Site settings → Environment. Optional: sync from a local file using
  opendeploy env sync netlify --file .env --project-id <SITE_ID> --yes
In CI, prefer opendeploy with --gha and provider credentials.
```
  </article>
</DocsLayout>
